...
async function loadContracts() {
  ...
  return (contracts || []).map((contract) => ({
    ...
    bookingRequestId: contract.booking_request_id || undefined,
    ...
    contractAmount: Number(contract.contract_amount ?? contract.monthly_amount ?? 0),
...
async function persistContracts(value: any[]) {
  ...
  await replaceSimpleRows('contracts', value.map((contract) => ({
    ...
    booking_request_id: contract.bookingRequestId || null,
