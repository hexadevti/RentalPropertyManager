-- Refactor notification templates/runtime for structured metadata and HTML/text handling.
...
set event_type = coalesce(event_type, 'general');

update public.notification_templates
set content_type = coalesce(content_type, 'html');
...
alter table public.notification_templates
  alter column content_type set default 'html';
...
alter table public.notification_templates
  ...
  check (event_type in ('general', 'tasks', 'user-access'));

alter table public.notification_templates
  drop constraint if exists notification_templates_content_type_check;

alter table public.notification_templates
  add constraint notification_templates_content_type_check
  check (content_type in ('html', 'text'));

create index if not exists idx_notification_templates_tenant_event_channel
  on public.notification_templates(tenant_id, event_type, channel);
...
alter table public.notification_deliveries
  drop constraint if exists notification_deliveries_content_type_check;

alter table public.notification_deliveries
  add constraint notification_deliveries_content_type_check
  check (content_type in ('html', 'text'));
