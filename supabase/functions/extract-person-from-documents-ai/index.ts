...
const ERROR_KEYS = {
  ...
  atLeastOneImageRequired: 'extract_person_at_least_one_image_required',
  maxImagesExceeded: 'extract_person_max_images_exceeded',
  unexpectedError: 'extract_person_unexpected_error',
} as const
...
const ERROR_MESSAGES = {
  pt: {
    ...
    atLeastOneImageRequired: 'Ao menos uma imagem valida e obrigatoria.',
    maxImagesExceeded: 'Voce pode enviar no maximo 6 imagens por extracao.',
    unexpectedExtractionError: 'Erro inesperado ao extrair dados da pessoa.',
  },
  en: {
    ...
    atLeastOneImageRequired: 'At least one valid image is required.',
    maxImagesExceeded: 'You can send up to 6 images per extraction.',
    unexpectedExtractionError: 'Unexpected error while extracting person data.',
  },
} as const
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (images.length === 0) {
      return jsonResponse({ error: ERROR_MESSAGES.en.atLeastOneImageRequired, errorKey: ERROR_KEYS.atLeastOneImageRequired }, 400)
    }

    if (images.length > 6) {
      return jsonResponse({ error: ERROR_MESSAGES.en.maxImagesExceeded, errorKey: ERROR_KEYS.maxImagesExceeded }, 400)
    }
