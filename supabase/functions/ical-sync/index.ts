...
const ERROR_KEYS = {
  ...
  serverConfigError: 'edge_server_config_error',
  missingAuthorization: 'edge_missing_authorization',
  invalidAuthToken: 'edge_invalid_auth_token',
  accessDenied: 'ical_sync_access_denied',
  tenantContextNotFound: 'ical_sync_tenant_context_not_found',
  loadPropertiesFailed: 'ical_sync_load_properties_failed',
  loadFeedsFailed: 'ical_sync_load_feeds_failed',
...
Deno.serve(async (req) => {
  try {
    ...
    if (req.method !== 'POST') return jsonResponse({ error: 'Method not allowed', errorKey: ERROR_KEYS.methodNotAllowed }, 405)
    ...
    if (!supabaseUrl || !supabaseAnonKey || !supabaseServiceRoleKey) {
      return jsonResponse({ error: 'Server configuration error', errorKey: ERROR_KEYS.serverConfigError }, 500)
    }

    const authorization = req.headers.get('Authorization')
    if (!authorization) return jsonResponse({ error: 'Missing Authorization header', errorKey: ERROR_KEYS.missingAuthorization }, 401)
  ...
  // Auth + tenant resolution
    ...
    if (authError || !authData.user) {
      return jsonResponse({ error: authError?.message ?? 'Invalid user token', errorKey: authError?.message ? undefined : ERROR_KEYS.invalidAuthToken }, 401)
    }
    ...
    if (!isPlatformAdmin && !isApprovedAdmin) {
      return jsonResponse({ error: 'Access denied', errorKey: ERROR_KEYS.accessDenied }, 403)
    }
    ...
    if (!tenantId) return jsonResponse({ error: 'Tenant context not found', errorKey: ERROR_KEYS.tenantContextNotFound }, 400)
  ...
  // Load properties
    ...
    if (propsError) return jsonResponse({ error: propsError.message, errorKey: ERROR_KEYS.loadPropertiesFailed }, 500)
  ...
  // Load iCal feeds from dedicated table
    ...
    if (feedsError) return jsonResponse({ error: feedsError.message, errorKey: ERROR_KEYS.loadFeedsFailed }, 500)
