-- Migration 206: guest password recovery and stronger password policy
...
begin
  ...
  if v_phone = '' then
    raise exception 'Telefone e obrigatorio';
  end if;
  if length(coalesce(p_password, '')) < 8 then
    raise exception 'A senha deve ter pelo menos 8 caracteres';
  end if;
  if coalesce(p_password, '') !~ '[A-Za-z]' or coalesce(p_password, '') !~ '[0-9]' then
  ...
  if not exists (
    ...
    raise exception 'Portal nao disponivel para este tenant';
  end if;
  ...
  if found then
    if coalesce(v_guest.password_hash, '') <> '' then
      raise exception 'Ja existe cadastro para este e-mail';
    end if;
...
create or replace function public.portal_guest_reset_password(
  p_tenant_id uuid,
  p_email text,
  p_phone text,
  p_new_password text
...
grant execute on function public.portal_guest_reset_password(uuid, text, text, text) to anon, authenticated;
